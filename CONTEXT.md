...
- Shell scripts and **GitHub Actions** build and push apps (RangersInfo on push + daily cron).
...
| Docker actions | `.github/actions/run-docker-rangersinfo/`, `run-docker-wc-chase/` |

## Local dev

- Copy or create `.env` with `TIDBYT_API_TOKEN`, `TIDBYT_DEVICE_ID` (and optional Kyle/Jason vars).
- Run `./build-rangersinfo.sh` or `./build-wcchase.sh` to build + push.
...
- Data sources used: MLB Stats API (`statsapi.mlb.com`), Fangraphs playoff odds API.
